...
class GAUserLevelController extends ApiController {
	...
	public function load(Request $request)
	{
        ...
        $id = 0;
        $user_id = 0; // filter records based on user id
        $status = 2; // 0: disabled, 1: enabled, 2: both
        $isapproved = 2; // 0: unapproved, 1: approved, 2: both
        $isfeatured = 3; // 0: normal, 1: featured, 2: premium, 3: all
        $iscomments = 2; // 0: disabled, 1: enabled
        $isadult = 2; // 0: normal, 1: adult
        $type = 0;  // 0: all
        $term = ""; // search term
        $order = "id";
        $direction = "desc";
        ...
        foreach($data as $item)
        {
           ...
           if(isset($item->status))
              $status = $item->status;
           if(isset($item->isapproved))
              $isapproved = $item->isapproved;
           if(isset($item->isfeatured))
              $isfeatured = $item->isfeatured;      
           if(isset($item->iscomments))
              $iscomments = $item->iscomments;          
           if(isset($item->isadult))
           ...
           if(isset($item->issummary))
              $isSummary = $item->issummary;
           if(isset($item->isDropdown))
              $isDropdown = $item->isDropdown;
           if(isset($item->thumbfilter))
              $thumbfilter = $item->thumbfilter;
           
        }
        
        $query = Blog::orderBy($order, $direction);
        ...
        if ($iscomments != 2)
          $query->where('iscomments', $iscomments);   
        if ($isadult != 2)
          $query->where('isadult', $isadult);   
        ...
        $data = null;
        if($isSummary)
           $data = $query->get(['id', 'title', 'url_caption', 'introduction', 'description', 'type', 'isfeatured', 'status','iscomment', 'isadult', 'comments']);
        else
           $data =  $query->get();
       
        return Response::json(['status' => 200, 'query' => $query->toSql(), 'records' => $records, 'posts' => $data]);
	}
